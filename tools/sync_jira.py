...
def fetch_issues(project_key):
    ...
    fields = "summary,status,issuetype,priority,parent,description,timetracking"
...
def main():
    ...
    project_key = "DC"
    output_file = "docs/plans/case 03 - implementation.md"
    ...
    output = []
    output.append("# Case 03: Data Center Digital Twin (L1) — Implementation Plan")
    output.append("")
    ...
    for epic in epics:
        active_children = [
            c for c in epic["children"] if c["status"].lower() in active_statuses
        ]
    ...
    for epic in active_epics:
        ...
        # Sort children
        epic["active_children"].sort(key=lambda x: (x["priority_val"], x["key"]))
